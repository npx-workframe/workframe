...
export function ConciergeFlow({ projectName, onComplete, inviteToken = '', inviteEmail = '' }: ConciergeFlowProps) {
  ...
  async function pickMode(mode: string) {
    ...
    try {
      ...
      if (mode === 'single_user_local') {
        ...
        await workframeAuthApi.localBootstrap(
          displayName || 'Owner',
          adminEmail.trim() || inviteEmail.trim() || 'owner@local',
        )
        const me = await workframeAuthApi.getMe()
        setWorkspaceId(me.current_workspace?.id || me.default_workspace?.id || '')
  ...
  async function continueFromSmtp() {
    ...
    const steps = buildWizardSteps(deploymentMode, modeChosen, isInvitee)
    const integrationsIdx = steps.findIndex((s) => s.id === 'integrations')
    if (integrationsIdx >= 0 && visitedMaxIndex >= integrationsIdx) {
      setStep('integrations')
      return
    }
  ...
  switch (step) {
    case 'intro':
      footer = (
        <Button disabled={busy} onClick={() => setStep('welcome')}>
          Get started
        </Button>
      )
      break
    ...
    case 'integrations':
      footer = (
        <>
          <Button variant="outline" disabled={busy} onClick={() => void skipToBilling()}>
            Skip
          </Button>
          <Button disabled={busy} onClick={() => void saveIntegrations()}>
            Continue
          </Button>
        </>
      )
    ...
    case 'billing':
      footer = (
        <Button disabled={busy} onClick={() => void saveBilling()}>
          Continue
        </Button>
      )
      break
    case 'workframe':
      footer = (
        <Button disabled={busy} onClick={() => void saveWorkframe()}>
          Continue
        </Button>
      )
      break
    ...
    case 'providers':
      if (workspaceId) {
        footer = (
          <Button disabled={busy} onClick={() => void saveProviders()}>
            Continue
          </Button>
        )
      }
    ...
    case 'invites':
      footer = (
        <>
          <Button variant="outline" disabled={busy} onClick={() => void finishInstall()}>
            {busy ? 'Finishing…' : 'Skip'}
          </Button>
          <Button disabled={busy} onClick={() => void sendInvites()}>
    ...
    case 'publish':
      footer = (
        <>
          <Button variant="outline" disabled={busy} onClick={() => void testPublicUrl()}>
            Test connection
          </Button>
          <Button
