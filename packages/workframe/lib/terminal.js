...
import { QUESTION_TIMEOUT_MS } from './package-info.js';
...
export function createTerminalDialogue({
  ...
  function stop(reason) {
    ...
    const error = new InputEndedError(terminalReason);
    if (!controller.signal.aborted) controller.abort(error);
    settlePending({ error });
    if (!closed) rl.close();
  }

  const onSigint = () => stop('interrupt');
  const onInputEnd = () => stop('eof');
  const onClose = () => {
    closed = true;
  ...
  return {
    ...
    ask(prompt, { timeoutMs = QUESTION_TIMEOUT_MS } = {}) {
      ...
      return new Promise((resolve, reject) => {
        pending = { resolve, reject, timer: setTimeout(() => stop('timeout'), timeoutMs) };
        try {
          rl.question(prompt, (answer) => settlePending({ answer }));
        } catch (error) {
          if (error?.code === 'ERR_USE_AFTER_CLOSE') stop(terminalReason || 'eof');
          else settlePending({ error });
        }
      });
    },
    close() {
