...
export function avatarCatalogPaths(root) {
  return [
    ...
    // legacy paths (pre-unified avatars/)
...
export function removeAgentRecord(root, profile) {
  ...
  removeRouteRecord(root, profile);
}

export function routesRegistryPath(root) {
  return path.join(workframeDir(root), 'routes.json');
}

export function loadRoutesRegistry(root) {
  const file = routesRegistryPath(root);
  if (!fs.existsSync(file)) {
    return { version: 1, default_profile: '', routes: [] };
  }
  return JSON.parse(fs.readFileSync(file, 'utf8'));
}

export function saveRoutesRegistry(root, data) {
  ensureWorkframeDir(root);
  fs.writeFileSync(routesRegistryPath(root), `${JSON.stringify(data, null, 2)}\n`);
}

export function removeRouteRecord(root, profile) {
  const registry = loadRoutesRegistry(root);
  const before = registry.routes?.length ?? 0;
  if (registry.routes) {
    registry.routes = registry.routes.filter((r) => r.profile !== profile);
    if (registry.routes.length !== before) {
      saveRoutesRegistry(root, registry);
    }
  }
}

export function upsertRouteRecord(root, profile, entry) {
  const registry = loadRoutesRegistry(root);
  registry.routes = registry.routes || [];
  const idx = registry.routes.findIndex((r) => r.profile === profile);
  if (idx >= 0) {
    registry.routes[idx] = { ...registry.routes[idx], ...entry };
  } else {
    registry.routes.push(entry);
  }
  saveRoutesRegistry(root, registry);
}
