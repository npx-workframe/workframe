...
Chat interaction refinement was also completed in Docker DogFood: reply/steer context stays inside the existing composer toolbar, reaction controls and the picker are borderless/flush, reaction hover details identify members, @mentions combine avatar-backed contacts and agents, new image attachments use workspace `uploads/`, storage filenames are not printed in chat, row rhythm is more spacious, and the message marker rail maps the full viewport with immediate jumps. The API suite is now 105 tests; the full web/API/Electron production build passes.
