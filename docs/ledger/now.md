...
- Reply references now use the shared button and icon system as compact two-line neo cards with an explicit author, preview, and jump affordance. The minimap clusters at the panel center instead of impersonating a second full-height scrollbar, caps very long histories at representative markers while retaining the active message, and keeps jumps immediate. Message rows use the larger symmetric spacing interval requested in the live review.
- Verification: 105 API tests and the full web/Python/Electron production build pass. Docker DogFood proof covered reply preview/cancel without composer squeeze, avatar-backed mention results, filename-free image rendering, the centered minimap and instant end-to-end jumps, exact emoji persistence across reload, and cleanup. A live migration-version collision with the existing run-ledger migration was caught during browser testing and fixed by assigning reactions migration 15.
