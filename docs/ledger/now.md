...
- Reply references now use the shared button and icon system as compact two-line neo controls with an explicit author, preview, and jump affordance; they stay transparent at rest with a tight leading inset and gain relief only on hover. The message navigator maps representative markers across the conversation's full height while retaining the active message, keeps idle ticks short and subdued, extends active/hovered ticks, and opens a themed author/time/message preview instead of a native browser tooltip. Message rows use the larger 16px symmetric spacing interval requested in the live review, and dock panel tabs remain flush until hovered.
