...
- The marker rail now spans the message viewport, so marker position communicates chat position; jumps are immediate rather than delayed by smooth scrolling. Message rows use the larger symmetric spacing interval requested in the live review.
- Verification: 105 API tests and the full web/Python/Electron production build pass. Docker DogFood proof covered reply preview/cancel without composer squeeze, avatar-backed mention results, filename-free image rendering, the distributed marker rail and instant jump, exact emoji persistence across reload, and cleanup. A live migration-version collision with the existing run-ledger migration was caught during browser testing and fixed by assigning reactions migration 15.
