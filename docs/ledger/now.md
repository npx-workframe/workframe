...
- Reply references now use the shared button and icon system as compact two-line neo controls with an explicit author, preview, and jump affordance; they stay transparent at rest with a tight leading inset and gain relief only on hover. The minimap clusters at the panel center instead of impersonating a second full-height scrollbar, caps very long histories at representative markers while retaining the active message, and keeps jumps immediate. Message rows use the larger 16px symmetric spacing interval requested in the live review, and dock panel tabs remain flush until hovered.
