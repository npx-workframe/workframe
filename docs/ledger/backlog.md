...
The bounded Socratic CLI campaign is defined in [`specs/WF-CLI-001/spec.md`](specs/WF-CLI-001/spec.md). `WF-CLI-001` is in **review** after a bounded remediation of the third independent review findings: terminal cancellation now reaches HTTP verification and asynchronous Codex/Claude children; hedged runtime choices remain unresolved; account-backed and API-key-backed paths disclose distinct billing and credential sources; inference children exclude Docker and SSH authority handles; and the implementation branch has been synchronized with current `main`. Implementer evidence records 43 passing package tests and a clean packed `workframe@0.2.1` install. Exact-head repository CI and independent reproduction remain reviewer work. `WF-CLI-002` remains dependency-gated. Work remains confined to `packages/workframe/**`; no package has been published and no existing installation has been changed.
