...
The bounded Socratic CLI campaign is defined in [`specs/WF-CLI-001/spec.md`](specs/WF-CLI-001/spec.md). `WF-CLI-001` is back in **todo** after independent review of draft PR #9. The current candidate repairs verification cancellation, account/API-key separation, structured response parsing, and inference-child environment authority, but candidate parsing can still select a path from descriptive or negative-context mentions and can select an unrelated remaining provider after mis-scoped exclusions. Windows production child termination is not proven fail-closed, and exact-head CI remains red. `WF-CLI-002` remains dependency-gated. Work remains confined to `packages/workframe/**`; no package has been published and no existing installation has been changed.
